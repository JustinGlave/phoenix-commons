...
| **8a** | ValveMasterTool / Phoenix Master Tool | `phase-8a-valvemaster-retrofit` | Not started — **commons-backed architecture alignment + build hardening + updater/theme/widget facades**. The v1.1.0 release already shipped the System A theme in `phoenix_style.qss` (see `WAVE_8A_VALVEMASTER_PREFLIGHT_AUDIT.md`), so the retrofit is NOT a visible theme swap. Expected visible change ≈ 0% (Phoenix-CAD profile). Operator-gated; doctrinal cooldown floor 2026-06-02 (14 days after Phase 3B's 2026-05-19 merge). |
